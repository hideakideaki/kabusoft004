...
STRATEGY_ID = "worker_13"
STRATEGY_NAME = "worker_13"
...
def _worker_07_filter(df: pd.DataFrame) -> pd.Series:
    return (
...
def _worker_03_filter(df: pd.DataFrame) -> pd.Series:
    return (
        (df["volatility_20"] < 0.03)
        & (df["range_pct"] > 0.035)
...
def _worker_09_filter(df: pd.DataFrame) -> pd.Series:
    return (
        (df["ret_20"] > 0.08)
        & (df["rebound_strength"] > 0.10)
        ...
        & (df["ret_1"] < 0.0)
        & (df["gap_open"] < 0.01)
        & (df["intraday_return"] > -0.015)
        & (df["volume_ratio_20"] > 0.8)
        & (df["volume_ratio_20"] < 2.5)
        & (df["volatility_20"] < 0.05)
    )
...
def generate_signals(
    ...
    candidate_df["worker_07_bonus"] = _worker_07_filter(candidate_df).astype(float)
    candidate_df["worker_03_bonus"] = _worker_03_filter(candidate_df).astype(float)
    candidate_df["worker_09_bonus"] = _worker_09_filter(candidate_df).astype(float)
    candidate_df["agreement_bonus"] = (
        candidate_df[["worker_07_bonus", "worker_03_bonus", "worker_09_bonus"]].sum(axis=1) >= 2
    ).astype(float)
    ...
    candidate_df["score"] = (
        ...
        + candidate_df["worker_07_bonus"] * 0.01
        + candidate_df["worker_03_bonus"] * 0.02
        + candidate_df["worker_09_bonus"] * 0.02
        + candidate_df["agreement_bonus"] * 0.03
    )
