...
STRATEGY_ID = "worker_10"
STRATEGY_NAME = "worker_10"
...
def generate_signals(features: pd.DataFrame, config: dict, holding_days: int, model_dir: Path | None = None):
    ...
    while start < len(dates):
        ...
        candidate_df["rule_score"] = (
            candidate_df["range_pct"] * 6.0
            + candidate_df["volume_ratio_20"] * 0.5
            + candidate_df["ret_20"] * 8.0
            + candidate_df["rebound_strength"] * 4.0
            + candidate_df["intraday_return"] * 20.0
            - candidate_df["ret_5"].abs() * 10.0
        )
        candidate_df["score"] = (
            candidate_df["logistic_score"] * 0.30
            + candidate_df["forest_score"] * 0.35
            + candidate_df["rule_score"] * 0.20
            + candidate_df["event_bonus"] * 0.10
            + candidate_df["pullback_bonus"] * 0.15
        )
        candidate_df = (
