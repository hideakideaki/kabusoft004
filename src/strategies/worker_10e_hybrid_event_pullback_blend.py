...
STRATEGY_ID = "worker_10e"
STRATEGY_NAME = "worker_10e"
...
def _event_filter(df: pd.DataFrame) -> pd.Series:
    return (
        (df["volatility_20"] < 0.036)
        & (df["range_pct"] > 0.025)
        & (df["volume_ratio_20"] > 1.1)
        & (df["ret_1"] > -0.004)
        & (df["breakout_strength"] > -0.027)
    )
...
def _pullback_filter(df: pd.DataFrame) -> pd.Series:
    return (
        ...
        & (df["breakout_strength"] > -0.038)
        & (df["ret_5"] < -0.02)
        & (df["ret_5"] > -0.09)
        & (df["ret_1"] < 0.0)
        & (df["gap_open"] < 0.009)
        & (df["intraday_return"] > -0.001)
        & (df["volume_ratio_20"] > 0.9)
        & (df["volume_ratio_20"] < 2.1)
        & (df["volatility_20"] < 0.042)
    )
...
def _select_blended(
    ...
    while len(selected_rows) < limit and not ordered.empty:
        ...
        for idx, row in ordered.iterrows():
            overlap_bonus = float(row["both_bonus"]) * 0.08
            candidate_value = float(row["score"]) + overlap_bonus
...
def generate_signals(features: pd.DataFrame, config: dict, holding_days: int, model_dir: Path | None = None):
    ...
    while start < len(dates):
        ...
        candidate_df["rule_score"] = (
            candidate_df["range_pct"] * 4.8
            + candidate_df["volume_ratio_20"] * 0.42
            + candidate_df["ret_20"] * 6.2
            + candidate_df["rebound_strength"] * 3.2
            + candidate_df["intraday_return"] * 13.0
            - candidate_df["ret_5"].abs() * 8.2
            - candidate_df["volatility_20"] * 9.0
        )
        candidate_df["score"] = (
            ...
            + candidate_df["both_bonus"] * 0.02
        )

        blended_groups = []
        for _, date_group in candidate_df.groupby("date", sort=False):
            blended_groups.append(
                _select_blended(date_group, return_matrix, lookback_days=20, limit=3)
            )
        candidate_df = pd.concat(blended_groups, ignore_index=True)
