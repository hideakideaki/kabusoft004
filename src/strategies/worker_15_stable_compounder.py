...
STRATEGY_ID = "worker_15"
STRATEGY_NAME = "worker_15"
STRATEGY_TYPE = "ml_based"
BACKTEST_OVERRIDES = {
    "max_positions": 4,
    "top_signals_per_day": 4,
    "max_new_positions_per_day": 1,
    "capital_deployment_ratio": 0.45,
    "stop_loss_pct": 0.05,
...
STABLE_FEATURE_COLUMNS = FEATURE_COLUMNS + [
    "market_breadth_20",
    "market_breadth_5",
    "market_median_ret_5",
...
def _stable_filter(df: pd.DataFrame) -> pd.Series:
    return (
        (df["market_breadth_20"] > 0.46)
        & (df["market_breadth_5"] > 0.40)
        & (df["market_median_ret_5"] > -0.015)
        & (df["market_median_drawdown_20"] > -0.10)
        & (df["ret_20"] > 0.045)
        & (df["ret_20"] < 0.24)
        & (df["ret_5"] > -0.035)
        & (df["ret_5"] < 0.03)
        ...
        & (df["rebound_strength"] > 0.06)
        & (df["range_pct"] < 0.04)
        & (df["range_stability_20"] < 0.038)
        & (df["gap_open"].abs() < 0.02)
        & (df["intraday_return"] > -0.012)
        & (df["intraday_return"] < 0.03)
        & (df["volume_ratio_20"] > 0.80)
        & (df["volume_ratio_20"] < 2.20)
        & (df["up_day_ratio_20"] > 0.50)
        & (df["trend_efficiency_20"] > 0.16)
        & (df["distance_from_sma20"] > -0.025)
        & (df["distance_from_sma20"] < 0.08)
        & (df["sma_trend_20_60"] > 0.0)
        & (df["close_position_20"] > 0.48)
...
def _add_market_state_features(features: pd.DataFrame) -> pd.DataFrame:
    ...
    rolling_high = grouped["high"].transform(
        lambda series: series.rolling(20).max().shift(1)
    )
    rolling_low = grouped["low"].transform(
        lambda series: series.rolling(20).min().shift(1)
    )
...
def generate_signals(
    ...
    stable_target_column = f"target_stable_{holding_days}"
    threshold = 0.015 if holding_days <= 10 else 0.03
    augmented[stable_target_column] = (
        augmented[f"future_return_{holding_days}"] > threshold
    ).astype(float)
    usable = augmented.dropna(subset=STABLE_FEATURE_COLUMNS + [stable_target_column]).copy()
    ...
    while start < len(dates):
        ...
        logistic.fit(train_df[STABLE_FEATURE_COLUMNS], train_df[stable_target_column].astype(int))
        boosting.fit(train_df[STABLE_FEATURE_COLUMNS], train_df[stable_target_column].astype(int))
        forest.fit(train_df[STABLE_FEATURE_COLUMNS], train_df[stable_target_column].astype(int))

        candidate_df = test_df[_stable_filter(test_df)].copy()
        if candidate_df.empty:
            start += step_days
            fold_index += 1
            continue

        logistic_score = logistic.predict_proba(test_df[STABLE_FEATURE_COLUMNS])[:, 1]
        boosting_score = boosting.predict_proba(test_df[STABLE_FEATURE_COLUMNS])[:, 1]
        forest_score = forest.predict_proba(test_df[STABLE_FEATURE_COLUMNS])[:, 1]
        ...
        candidate_df["stability_score"] = (
            candidate_df["trend_efficiency_20"] * 12.0
            + candidate_df["up_day_ratio_20"] * 3.0
            + candidate_df["sma_trend_20_60"] * 10.0
            + candidate_df["ret_20"] * 10.0
            + candidate_df["distance_from_sma20"] * 5.0
            - candidate_df["downside_vol_20"] * 20.0
            - candidate_df["volatility_20"] * 16.0
            - candidate_df["range_stability_20"] * 8.0
            - (candidate_df["volume_ratio_20"] - 1.05).abs() * 1.2
            - (candidate_df["close_position_20"] - 0.72).abs() * 1.5
            - candidate_df["gap_open"].abs() * 6.0
        )
        candidate_df["score"] = (
            candidate_df["logistic_score"] * 0.12
            + candidate_df["boosting_score"] * 0.28
            + candidate_df["forest_score"] * 0.15
            + candidate_df["stability_score"] * 0.45
        )
