...
STRATEGY_ID = "worker_17d"
STRATEGY_NAME = "worker_17d"
...
MIN_META_SCORE_FOR_DEPLOY = 0.65
SINGLE_META_SCORE_GAP = 0.28
...
def _realized_fold_score(metrics: dict) -> float:
    return (
        float(metrics["sharpe"]) * 0.60
        + max(float(metrics["cagr"]), 0.0) * 1.25
        + float(metrics["win_rate"]) * 0.30
        + float(metrics["max_drawdown"]) * 0.70
    )
...
def _window_ranking_score(selected_main: list[dict], selection_mode: str) -> float:
    ...
    mode_bonus = 0.10 if selection_mode == "single" else 0.0
    return avg_meta_score + avg_heuristic_score * 0.08 + mode_bonus
